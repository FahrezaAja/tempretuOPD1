...
<div class="max-w-6xl mx-auto p-6">
    ...
    <h1 class="text-3xl font-bold mb-6 text-indigo-700">Kelola Struktur Organisasi</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-gray-800">Update Struktur Organisasi</h2>

        <form action="{{ route('strukturOrganisasiAdmin.update', $strukturorganisasi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
                ...
                @if($strukturorganisasi->image)
                    <img src="{{ asset('storage/' . $strukturorganisasi->image) }}" alt="Gambar"
    ...
    <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-red-700">Hapus Struktur Organisasi</h2>
        <p class="mb-4 text-gray-700">Klik tombol di bawah untuk menghapus Struktur Organisasi ini secara permanen.</p>

        <form action="{{ route('strukturOrganisasiAdmin.destroy', $strukturorganisasi->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus Struktur ORganisasi ini?');">
            @csrf
            @method('DELETE')
            <button type="submit"
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-gray-800">Tambah Struktur Organisasi Baru</h2>

        <form action="{{ route('strukturOrganisasiAdmin.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div>
                <label class="block font-medium text-gray-700 mb-1">Gambar</label>
                <input type="file" name="image"
