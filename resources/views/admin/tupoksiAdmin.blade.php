...
@section('title', 'Manajemen Tugas Pokok & Fungsi')
@section('content')
<div class="max-w-6xl mx-auto p-6">

    <h1 class="text-3xl font-bold mb-6 text-indigo-700">Kelola Tugas Pokok & Fungsi</h1>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-gray-800">Update Tupoksi</h2>

        <form action="{{ route('tupoksiAdmin.update', $tupoksi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="tugaspokok_edit" name="tugas_pokok" rows="6" required
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none">{{ old('tugas_pokok', $tupoksi->tugas_pokok) }}</textarea>
            </div>

            <div>
                <label class="block font-medium text-gray-700 mb-1">Fungsi</label>
                <textarea id="fungsi_edit" name="fungsi" rows="6" required
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none">{{ old('deskripsi', $tupoksi->fungsi) }}</textarea>
            </div>
    ...
    <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-red-700">Hapus Tugas Pokok & Fungsi</h2>
        <p class="mb-4 text-gray-700">Klik tombol di bawah untuk menghapus Tupoksi ini secara permanen.</p>

        <form action="{{ route('tupoksiAdmin.destroy', $tupoksi->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus Tupoksi?');">
            @csrf
            @method('DELETE')
            <button type="submit"
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        <h2 class="text-xl font-semibold mb-4 text-gray-800">Tambah Tupoksi Baru</h2>

        <form action="{{ route('tupoksiAdmin.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
