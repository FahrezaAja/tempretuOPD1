...
<div class="max-w-6xl mx-auto p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
        ...
        <form action="{{ route('bidangPolitikAdmin.update', $bidangpolitik->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
    ...
    <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        ...
        <form action="{{ route('bidangPolitikAdmin.destroy', $bidangpolitik->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus Bidang Politik?');">
            @csrf
            @method('DELETE')
            <button type="submit"
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        ...
        <form action="{{ route('bidangPolitikAdmin.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
