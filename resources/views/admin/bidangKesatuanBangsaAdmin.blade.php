    ...
    <div class="max-w-6xl mx-auto p-6">
        ...
        @if($bidangkesatuanbangsa)
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
                ...
                <form action="{{ route('bidangKesatuanBangsaAdmin.update', $bidangkesatuanbangsa->id) }}" method="POST"
            ...
            <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
                ...
                <p class="mb-4 text-gray-700">Klik tombol di bawah untuk menghapus Bidang Kesatuan Bangsa ini secara permanen.
                </p>

                <form action="{{ route('bidangKesatuanBangsaAdmin.destroy', $bidangkesatuanbangsa->id) }}" method="POST"
                    onsubmit="return confirm('Yakin ingin menghapus Bidang Kesatuan Bangsa?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
                ...
                <form action="{{ route('bidangKesatuanBangsaAdmin.store') }}" method="POST" enctype="multipart/form-data"
                    class="space-y-5">
                    @csrf
