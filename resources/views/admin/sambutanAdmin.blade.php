...
<div class="max-w-6xl mx-auto p-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
        ...
        <form action="{{ route('katasambutan.update', $katasambutan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nama_kepala_badan" value="{{ old('nama_kepala_badan', $katasambutan->nama_kepala_badan) }}" required
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none">
            </div>
    ...
    <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
        ...
        <form action="{{ route('katasambutan.destroy', $katasambutan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus sambutan ini?');">
            @csrf
            @method('DELETE')
            <button type="submit"
