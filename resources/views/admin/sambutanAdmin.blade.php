...
@section('content')
    <div class="max-w-6xl mx-auto p-6">
        ...
        <h1 class="text-3xl font-bold mb-6 text-indigo-700">Kelola Kata Sambutan</h1>
        ...
        @if($katasambutan)
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-8 transition transform hover:scale-[1.01]">
                <h2 class="text-xl font-semibold mb-4 text-gray-800">Update Sambutan</h2>

                <form action="{{ route('katasambutan.update', $katasambutan->id) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul', $katasambutan->judul) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_opd" value="{{ old('nama_opd', $katasambutan->nama_opd) }}" required
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>

                    <div>
                        <label class="block font-medium text-gray-700 mb-1">Nama Kepala Badan</label>
                        <input type="text" name="nama_kepala_badan"
                            value="{{ old('nama_kepala_badan', $katasambutan->nama_kepala_badan) }}" required
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        @if($katasambutan->image)
                            <img src="{{ asset('storage/' . $katasambutan->image) }}" alt="Gambar"
            ...
            <div class="bg-red-50 rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
                <h2 class="text-xl font-semibold mb-4 text-red-700">Hapus Sambutan</h2>
                <p class="mb-4 text-gray-700">Klik tombol di bawah untuk menghapus sambutan ini secara permanen.</p>

                <form action="{{ route('katasambutan.destroy', $katasambutan->id) }}" method="POST"
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 transition transform hover:scale-[1.01]">
                <h2 class="text-xl font-semibold mb-4 text-gray-800">Tambah Sambutan Baru</h2>
