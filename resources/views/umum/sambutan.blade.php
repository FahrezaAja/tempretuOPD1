    ...
    <section x-data="{ show: false }" x-init="setTimeout(() => show = true, 200)"
        ...
        <div class="relative w-full md:w-1/2 flex justify-center items-center py-16 z-10
            ...
            <div class="relative z-10 overflow-hidden rounded-[40px] shadow-2xl">
                @if($katasambutan->image)
                    <img src="{{ asset('storage/' . $katasambutan->image) }}" alt="Kepala Badan" class="object-cover w-[300px] md:w-[400px] lg:w-[450px] rounded-[40px]
                                            transition-all duration-1000 ease-out transform"
                        :class="show ? 'scale-100' : 'scale-90 opacity-0'">
                @else
                    <img src="{{ asset('images/depan-kanan-orang.jpg') }}" alt="Kepala Badan" class="object-cover w-[300px] md:w-[400px] lg:w-[450px] rounded-[40px]
                                            transition-all duration-1000 ease-out transform"
                        :class="show ? 'scale-100' : 'scale-90 opacity-0'">
                @endif
            </div>
        ...
        <div class="relative w-full md:w-1/2 text-gray-800 px-8 md:px-16 py-20 space-y-6 z-10
            ...
            <p class="text-lg leading-relaxed text-gray-600 ck-content">
                {!! $katasambutan->deskripsi !!}
            </p>

            <div class="pt-4">
                <p class="font-bold text-lg text-indigo-700">KEPALA {{ $katasambutan->nama_opd }}</p>
