...
@section('content')
    ...
    <section x-data="heroSection()" x-init="init()"
        ...
        <div
            ...
            {{-- TEXT --}}
            <div :class="mobile ? 'flex flex-col justify-center items-center opacity-0 transition-opacity duration-700 fade-in' : ''"
                class="space-y-6 mt-10 md:mt-0 md:max-w-xl w-full px-4 md:px-0"
                :style="desktop ? 'transform: translate3d(' + textOffset + 'px, 0, 0);' : ''">

                <h1 class="text-4xl md:text-6xl font-extrabold leading-tight">
                    <span class="text-indigo-400">{{ $sampul ? $sampul->nama_opd : 'Nama OPD' }}</span><br>
                </h1>

                {{-- Deskripsi rapi (justified) --}}
                <div class="text-gray-200 text-lg leading-relaxed break-words whitespace-pre-line text-justify"
                    style="word-break: break-word; white-space: pre-wrap; line-height:1.7; margin:0; padding:0;">
    ...
    {{-- ================= SECTION 2 ================= --}}
    <section x-data="sectionTwo()" x-init="init()"
        ...
        {{-- KOTAK TEKS --}}
        <div :class="mobile ? 'opacity-0 transition-opacity duration-700 fade-in flex flex-col items-center mx-auto' : ''"
            class="relative w-full md:w-1/2 bg-black text-white py-16 px-8 md:px-12 flex flex-col justify-center rounded-r-[80px] z-10 shadow-2xl"
            :style="desktop ? 'transform: translate3d(' + textOffset + 'px, 0, 0);' : ''">

            <div class="absolute right-[-120px] top-1/2 transform -translate-y-1/2 
            ...
            {{-- Deskripsi rapi (justified) --}}
            <p class="text-gray-300 text-base leading-relaxed mt-6 max-w-md text-justify">
                @if($sambutan)
        ...
        {{-- GAMBAR --}}
        <div :class="mobile ? 'opacity-0 transition-opacity duration-700 fade-in' : ''"
            class="relative w-full md:w-1/2 flex justify-center items-center mt-16 md:mt-0 z-20 px-4"
            :style="desktop ? 'transform: translate3d(' + imageOffset + 'px, 0, 0);' : ''">
                        ...
                        <div class="p-5 space-y-3">
                            ...
                            <div class="pt-3">
                                <button class="text-indigo-600 font-semibold hover:underline"
                                    @click.stop="openNewsModal(news)">Baca Selengkapnya →</button>
                            </div>
                        </div>
                    </div>
                </template>
                        ...
                        <div class="grid grid-cols-3 gap-3 mt-4">
                            <template x-for="foto in selectedNews.foto_berita" :key="foto">
                                <img :src="foto" alt="Foto Berita"
                                    class="w-full h-32 object-cover rounded border border-gray-200" loading="lazy">
                            </template>
                        </div>
                    </template>
