...
<section x-data="heroSection()" x-init="init()"
    ...
    @if($sampul && $sampul->media && \Illuminate\Support\Facades\Storage::disk('public')->exists($sampul->media))
        ...
        @else
            <img src="{{ asset('storage/'.$sampul->media) }}" class="absolute inset-0 w-full h-full object-cover brightness-75" alt="Hero Background" loading="lazy">
        @endif
    @else
    ...
    {{-- Hero Content --}}
    <div class="relative z-30 flex flex-col-reverse md:flex-row items-center justify-between w-full px-6 sm:px-10 lg:px-24 text-white">
        {{-- TEXT --}}
        ...
        <template x-if="desktop">
            <div class="relative flex justify-center items-center" :style="'transform: translate3d(' + imageOffset + 'px, 0, 0);'">
...
{{-- ================= BERITA TERBARU ================= --}}
<section x-data="newsCarousel()" class="relative bg-gradient-to-b from-black via-gray-900 to-black py-24 px-6 lg:px-20 text-white overflow-hidden">
    ...
    <div class="relative w-full overflow-hidden">
        <div class="flex transition-transform duration-[1000ms] ease-in-out"
            ...
            <template x-for="(news, index) in newsList" :key="index">
                <div class="min-w-[350px] max-w-[350px] mx-3 bg-white rounded-3xl overflow-hidden shadow-lg cursor-pointer hover:shadow-2xl transition-transform duration-500">
                    <div class="relative overflow-hidden">
                        <img :src="news.image" alt=""
                             class="w-full h-56 object-cover transition-transform duration-500 hover:scale-105">
                        <template x-if="news.unggulan">
                            <span class="absolute top-4 left-4 bg-yellow-400 text-white px-3 py-1 text-xs font-semibold rounded-full shadow-md">
                                ⭐ Unggulan
                            </span>
                        </template>
                    </div>
    ...
    {{-- MODAL BERITA --}}
    <div x-show="openModal" x-transition.opacity.duration.300ms x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center bg-black bg-opacity-50 p-6" @click.self="closeNewsModal()">
                ...
                <template x-if="selectedNews.foto_berita && selectedNews.foto_berita.length > 0">
                    <div class="grid grid-cols-3 gap-3 mt-4">
                        <template x-for="foto in selectedNews.foto_berita" :key="foto">
                            <img :src="foto" alt="Foto Berita" class="w-full h-32 object-cover rounded border border-gray-200">
                        </template>
                    </div>
                </template>
...
<section x-data="galleryModal()" class="relative bg-white py-28 px-6 lg:px-20 overflow-hidden text-gray-800">
    ...
    <div class="grid grid-cols-12 gap-6 md:gap-8">
        <template x-for="(item, index) in galleryList" :key="index">
            <div class="col-span-12 md:col-span-4 relative overflow-hidden rounded-[30px] shadow-xl group cursor-pointer" @click="openModal(item)">
                <img :src="item.image" :alt="item.title" class="w-full h-[300px] object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy">
    ...
    {{-- MODAL GALERI --}}
    <div x-show="isOpen" x-transition.opacity.duration.300ms x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="closeModal()">
        <div x-transition.scale.duration.300ms class="bg-white rounded-3xl shadow-2xl max-w-3xl w-full overflow-hidden flex flex-col relative" @click.stop>
            <button @click.stop="closeModal()" class="absolute top-3 right-3 text-gray-600 hover:text-red-500 bg-white/80 rounded-full p-2 shadow-md transition">
            ...
            {{-- DESKRIPSI --}}
            <div class="font-normal p-6 text-gray-700 text-base leading-relaxed max-h-48 overflow-y-auto whitespace-pre-line break-words border-t border-gray-100">
            ...
            if(this.desktop){
                window.addEventListener('scroll', () => {
                    window.requestAnimationFrame(() => {
                        this.textOffset = -window.scrollY * 0.85;
                        this.imageOffset = window.scrollY * 0.85;
                    });
                });
            } else {
                setTimeout(() => {
...
Alpine.data('sectionTwoParallax', () => ({
    ...
    Alpine.data('newsCarousel', ()=>({
        ...
        init(){
            const minItems=3;
            let tempNews=[...this.originalNews];
